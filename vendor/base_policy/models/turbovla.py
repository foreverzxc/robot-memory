...
class TurboVLA(nn.Module):
    ...
    def encode_text(self, instructions, device=None):
        """只编码一次语言部分（冻结 BERT + 投影）。

        同一指令在整段 episode 中输出恒定，可缓存复用，省掉每步重复的 BERT 前向。
        注意：完整语言 KV 无法缓存，因为融合层里文本 token 会注意力到视觉 token。
        """
        if device is None:
            device = next(self.parameters()).device
        return self.text_encoder(instructions, device=device)
